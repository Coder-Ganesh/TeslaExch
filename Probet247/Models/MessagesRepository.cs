    ...
    public class MessagesRepository
    {
        ...
        public IEnumerable<dlanalysisdata> GetAllMessages()
        {
            ...
            try
            {
                ...
                {
                    using (var cmd = new SqlCommand("SELECT distinct(betfair_id) , event_id from live_bet where dist_id='" + user_ids + "' AND status='' AND odds_type='MO'", con))
                    {
                        con.Open();
                            ...
                            while (reader.Read())
                            {
                                ...
                                using (var cmd1 = new SqlCommand("SELECT book_id,match_title,betfair_id,event_code,match_time,sport_id,league_id from matches where event_code='" + event_id + "' AND status='OPEN' ", con))
                                {
                                    ...
                                    if (reader1.HasRows)
                                    {
                                        ...
                                        string market_name = GetMArketName(event_id, betfair_id);
                                        using (var cmd2 = new SqlCommand("select amount,runner_pos from runner_cal where dist_id='" + user_ids + "' AND market_id='" + betfair_id + "'", con))
                                        {
                                        ...
                                        //string spn = GetSportname(sport_id);
