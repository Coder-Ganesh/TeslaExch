    ...
    public class Teensekhrepos
    {
        ...
        public IEnumerable<dlanalysisdata> GetAllMessages()
        {
            ...
            try
            {
                ...
                {
                    using (var cmd = new SqlCommand("SELECT distinct(betfair_id) , event_id from live_bet where sup_id='" + user_ids + "' AND status='' AND odds_type='TP'", con))
                    {
                        con.Open();
                            ...
                            while (reader.Read())
                            {
                                ...
                                using (var cmd1 = new SqlCommand("SELECT created , market_name from markets where betfair_id='" + betfair_id + "' AND status='activate' ", con))
                                {
                                    ...
                                    if (reader1.HasRows)
                                    {
                                        ...
                                        SqlCommand sqlLost = new SqlCommand("SELECT stakes,total_value,runner_posi,event_id FROM live_bet WHERE betfair_id='" + betfair_id + "' AND sup_id='" + user_ids + "' ", con);
