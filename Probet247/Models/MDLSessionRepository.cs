...
{
    public class MDLSessionRepository
    {
        public IEnumerable<dlanalysisdataS> GetAllMessagesS()
        {
            ...
            try
            {
                ...
                {
                    using (var cmd = new SqlCommand("SELECT distinct event_id,betfair_id from live_bet_new where md_id='" + user_ids + "' AND status='' AND odds_type='sess' ", con))
                    {
                        con.Open();
